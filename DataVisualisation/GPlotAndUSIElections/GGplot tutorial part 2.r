...
gg <- ggplot(midwest, aes(x=area, y=poptotal)) + 
  ...
  labs(title="Area Vs Population", y="Population", x="Area", 
       caption="Source: midwest", subtitle="X and Y axis Flipped") + theme(legend.position = "None")
...
gg <- ggplot(midwest, aes(x=area, y=poptotal)) + 
  ...
  geom_smooth(method="loess", se=F) + xlim(c(0, 0.1)) + ylim(c(0, 500000)) + 
  labs(title="Area Vs Population", y="Population", x="Area", 
       caption="Source: midwest", subtitle="Axis Scales Reversed") + theme(legend.position = "None")
...
# Facet wrap with free scales - each chart has its own scale.
...
img <-  readPNG(getURLContent("https://www.r-project.org/Rlogo.png"))
